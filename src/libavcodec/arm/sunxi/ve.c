...
static struct ve_mem *ion_malloc(int size)
{
    ...
    */
    ...
    //sprintf(buffer, "ION_IOC_ALLOCx: %d bytes\n", size);
    // printf("ION_IOC_ALLOCx: %d bytes\n", size);
	...
	{
		sprintf(buffer, "ION_IOC_ALLOCx failed: %d bytes", size);
		perror(buffer);
		free(imem);
		return NULL;
	}
...
struct ve_mem *ve_malloc(int size)
{
    ...
    printf("ve_malloc: %d bytes\n", size);
    size = (size + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);
    printf("ve_malloc new size: %d bytes\n", size);
...
void ve_flush_cache(struct ve_mem *mem)
{
	...
	if (ve.ion_fd != -1)
	{
        ...
        if (new_ioctl || ioctl(ve.ion_fd, ION_IOC_CUSTOM, &cache))  {
            if (ioctl(ve.ion_fd, ION_IOC_SUNXI_FLUSH_RANGE, &range))
