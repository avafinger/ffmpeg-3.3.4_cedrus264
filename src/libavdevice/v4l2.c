...
static const int desired_video_buffers = 4; // 256;
...
static int mmap_read_frame(AVFormatContext *ctx, AVPacket *pkt)
{
    ...
    if (atomic_load(&s->buffers_queued) == FFMAX(s->buffers / 8, 1)) {
        ...
        res = av_new_packet(pkt, frame_size /* @lex avafinger) */);
        if (res < 0) {
            av_log(ctx, AV_LOG_ERROR, "Error allocating a packet.\n");
            enqueue_buffer(s, &buf);
            return res;
        }
        memcpy(pkt->data, s->buf_start[buf.index], frame_size /* @lex avafinger) */);
        ...
        pkt->size     = frame_size /* @lex avafinger) */;
